...
3. If a subject is missing, propose a person/place/object, then accept its representation in Review queue. Shared names never merge entities automatically.
4. Open a source, select a passage in the text area, choose an accepted subject, and write what the source reports. Explain the representation. The resulting statement is pending and does not yet change accepted evidence.
5. In Review queue, inspect the cited excerpt and open the original source if needed. Enter a decision reason and accept or reject. “Accepted” confirms the representation, not its truth. New statements in this build have unknown time and unknown source independence.
6. Inspect records and their citations in Case register. Investigator notes follow the same proposal/review boundary. Timeline displays existing source occurrence information; it does not yet provide the complete time-editing or constraint solver workflow.
7. In Integrity & backup, verify byte hashes and the audit chain. Any mismatch blocks backup and original content access. A full portable backup is sensitive and unredacted.
